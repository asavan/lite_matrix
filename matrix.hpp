...
{
	if (size_stl != A.size_str) {throw std::exception("can't multiplay matrices");}
...
{
	int i,j;
	if (size_str< b.size_str || size_stl < b.size_stl) {throw std::exception("can't plus= matrices");}
...
cmatr<data_t>::cmatr(const char * file_name)
{
	...
	if(!is){std::string s ("can't open matrix file "); s+= file_name; throw std::exception(s.c_str());}
	size_str = 0;
	is>>(*this);
}
...
{
	if (newsize_str == 0) newsize_str = default_matrix_size;
	if (newsize_stl == 0) newsize_stl = newsize_str;
